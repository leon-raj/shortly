	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr != nil && !errors.Is(err, tt.wantErr) {
				t.Errorf("sanitizeURL() error = %v, wanted = %v", err, tt.wantErr)
			}
			if tt.wantParseErr && err == nil {
				t.Errorf("sanitizeURL() wanted parse error but got nil")
			}
			if err != nil && got != "" {
				t.Errorf("sanitizeURL() expected empty URL while error != nil, got %v", got)
			}
		})
	}
}
